...
export function PageHeader({ title, subtitle, actions, icon }: PageHeaderProps) {
  return (
    <header className="surface-card flex flex-col gap-4 p-4 sm:min-h-[5.75rem] sm:flex-row sm:items-center sm:justify-between sm:p-5">
      ...
      {actions && (
        <div className="flex shrink-0 flex-wrap items-center gap-2 sm:self-center">{actions}</div>
      )}
    </header>
  )
}
